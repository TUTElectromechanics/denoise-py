#!/usr/bin/python3
# -*- coding: utf-8 -*-
"""Denoise measured magnetic polarization (J = B - mu0*H) curves.
...
def scrub(sigma, path, show=False, verbose=False):
    """Denoise a magnetic polarization (J = B - mu0*H) curve.
...
#    # DEBUG
#    J = B - scipy.constants.mu_0*H
#    import matplotlib.pyplot as plt
#    plt.plot(B, color='#a0a0a0', linestyle='dashed')
#    plt.plot(pol)
#    plt.plot(J)
#    plt.show()
#    sys.exit(0)
    ...
    #
    b,a     = scipy.signal.butter(4, 0.05)  # Butterworth filter;  order, cutoff [fraction of Nyquist]
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="""Denoise measured magnetic polarization (J = B - mu0*H) curves.""", formatter_class=argparse.RawDescriptionHelpFormatter)
